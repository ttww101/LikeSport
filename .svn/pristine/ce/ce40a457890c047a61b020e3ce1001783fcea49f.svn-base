...
- (void)awakeFromNib
{
    [_cannelBtn setTitle:NSLocalizedStringFromTable(@"Cancel", @"InfoPlist", nil) forState:UIControlStateNormal];
    [_sureBtn setTitle:NSLocalizedStringFromTable(@"Yes", @"InfoPlist", nil) forState:UIControlStateNormal];
    /**背景*/
...
- (IBAction)removeBtnClick:(id)sender {
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        [self.delegate getSelectDate:nil type:DateTypeOfEnd];
    }];
}
...
- (IBAction)sureBtnClick:(id)sender {
    ...
    [self.delegate getSelectDate:self.selectDate type:DateTypeOfStart];
