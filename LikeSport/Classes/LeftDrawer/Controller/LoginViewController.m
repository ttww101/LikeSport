...
//        [KSLikeSportTool userThirdLoginWithParams:params WithCompleted:^(id result) {
...
- (void)login {
    ...
    if (_nameTextField.text.length < 6) {
        ...
        [KSKuaiShouTool userLoginWithName:_nameTextField.text andPassword:_passwordTextField.text WithCompleted:^(id result) {
            Login *last = [Login mj_objectWithKeyValues:result];
            if (last.ret_code == 0) {
                if (last.result.token.length > 0) {
                    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
                    
                    [userDefaults setValue:_nameTextField.text forKey:@"kUserName"];
                    [userDefaults setValue:_passwordTextField.text forKey:@"kPassword"];
