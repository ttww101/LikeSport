...
- (void)setData {
    [KSKuaiShouTool getCommentType:_type withMatchID:_matchID withCompleted:^(id result) {
...
//    NSLog(@"评论内容%@",_textField.text);
    if (_textField.text.length == 0) {
        ...
        [KSKuaiShouTool commentWithType:_type withMid:_matchID withReplyId:_replyid withContent:_textField.text withCompleted:^(id result) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.deleteBlock = ^(NSInteger commentId) {
        ...
        [KSKuaiShouTool deleteCommentWithId:commentId withCompleted:^(id result) {
            int ret_code = [[result objectForKey:@"ret_code"] integerValue];
            if (ret_code == 0) {
                MBProgressHUD *hud = [MBProgressHUD showHUDAddedTo:self.view animated:YES];
                [hud hideAnimated:YES afterDelay:1.f];
