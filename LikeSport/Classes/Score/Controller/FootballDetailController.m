...
-(void)setupfrist{
    ...
    if ([[TimeOfBootCount valueForKey:@"time"] intValue]==1) {
        
        [TimeOfBootCount setValue:@"2" forKey:@"time"];
        
        RAYNewFunctionGuideVC *vc = [[RAYNewFunctionGuideVC alloc]init];
        vc.titles = @[NSLocalizedStringFromTable(@"Learn the team info and follow the team", @"InfoPlist",nil),NSLocalizedStringFromTable(@"Follow the league , live score for the matches of the league will be pushed", @"InfoPlist",nil),];
        ...
        NSString* frame2=[NSString stringWithFormat:@"{{%f,%f},{100,80}}",[UIScreen mainScreen].bounds.size.width/10.0f,self.topView.hteamImage.center.y-40.0f+64];
...
- (void)setUpTopView {
    ...
    [KSKuaiShouTool getFootballBaseType:_type withMatchID:_matchID withCompleted:^(id result) {
        ...
        // 标题
        NSString* fullname=baseResult.matchtypefullname;
        NSArray *languages = [NSLocale preferredLanguages];
        UIFont *currentfont;

//        NSString *currentLanguage = [languages objectAtIndex:0];
//        if ([currentLanguage isEqualToString:@"zh-Hant-CN"]) {
//            
//          currentfont=[UIFont fontWithName:@"CourierNewPS-BoldMT" size:16.0f];
//            
//        }else{
//            
//          currentfont=[UIFont systemFontOfSize:16.0f];
//        
//        }
//        CGRect rect=[fullname boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, 40) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName: currentfont} context:nil];
//        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, rect.size.width, 40)];
...
//        self.navigationItem.titleView = label;
        ...
        NSString *hteamUrl = [[NSString alloc] initWithFormat:@"http://liansai.500.com/static/soccerdata/images/%@/teamsignnew_%li.png", _type==1?@"BasketBallTeamPic":@"TeamPic", (long)baseResult.hteam_id];
        if (_type == 1) hteamUrl = basketResult.hteamimgurl;
        ...
        NSString *cteamUrl = [[NSString alloc] initWithFormat:@"http://liansai.500.com/static/soccerdata/images/TeamPic/teamsignnew_%li.png",(long)baseResult.cteam_id];
        if (_type == 1) cteamUrl = basketResult.cteamimgurl;
...
//    three.wiki.mtype = _type;
    three.mtype = _type;
    three.mid = _matchID;
    three.title = NSLocalizedStringFromTable(@"News", @"InfoPlist", nil);
...
-(KSTopView *)topView {
    if (!_topView) {
        _topView = [KSTopView topView];
        _topView.frame = CGRectMake(0, 0, kSceenWidth, 145);
        if ([self isNotchScreen]) _topView.frame = CGRectMake(0, 0, kSceenWidth, 175);
        _topView.backgroundColor = KSBlue;
