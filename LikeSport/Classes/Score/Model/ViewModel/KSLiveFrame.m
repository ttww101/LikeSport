...
//    CGSize starttimeSize = [[NSString stringWithFormat:@"%li",_live.starttime] sizeWithAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:13]}];
    ...
    if (_live.type == 2 && !_live.isFollowView) {
        ...
        if ([_live.state isEqualToString:@"F"] || [_live.state isEqualToString:@"R"] || [_live.state isEqualToString:@"C"] || [_live.state isEqualToString:@"Q"] || [_live.state isEqualToString:@"T"] || [_live.state isEqualToString:@"Y"] || [_live.state isEqualToString:@"D"] || [_live.state isEqualToString:@"E"]) { // 网球完场时状态长度变小
            stateSize = CGSizeMake(45, 15);
        } else {
            stateSize = CGSizeMake(85, 15);
        }
    }
    else {
        stateSize = CGSizeMake(45, 15);
...
//    CGSize cteamnameSize = [_live.cteamname sizeWithAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:15]}];
  ...
  //  CGSize hSize = [_live.hteamname sizeWithAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:15]}];
        CGSize hSize = [_live.hteamname boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, 18.0f) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:15]} context:nil].size;
...
//    _cteamnameFrame = (CGRect){{cteamnameX,cteamnameY},cteamnameSize};
    ...
    if (_live.hascourts == 4) {
        ...
        CGFloat otX = kSceenWidth/2 - 40;
        CGFloat otY = CGRectGetMaxY(_halfXFrame) + 5;
        CGSize otSize = CGSizeMake(80, 15);
        _otFrame = (CGRect){{otX,otY},otSize};
