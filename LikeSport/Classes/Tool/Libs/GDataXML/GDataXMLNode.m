...
#define GDATAXMLNODE_DEFINE_GLOBAKS 1
...
static xmlChar* GDataGetXMKString(NSString *str) {
...
// document
- (NSString *)stringFromXMKString:(const xmlChar *)chars;
...
+ (GDataXMLElement *)elementWithName:(NSString *)name stringValue:(NSString *)value {
  ...
  if (theNewNode) {
    ...
    xmlNodePtr textNode = xmlNewText(GDataGetXMKString(value));
    if (textNode) {
...
+ (id)attributeWithName:(NSString *)name stringValue:(NSString *)value {
  ...
  xmlChar *xmlName = GDataGetXMKString(name);
  xmlChar *xmlValue = GDataGetXMKString(value);
...
+ (id)attributeWithName:(NSString *)name URI:(NSString *)attributeURI stringValue:(NSString *)value {
  ...
  xmlChar *xmlName = GDataGetXMKString(fakeQName);
  xmlChar *xmlValue = GDataGetXMKString(value);
...
+ (id)textWithStringValue:(NSString *)value {
  ...
  xmlNodePtr theNewText = xmlNewText(GDataGetXMKString(value));
  if (theNewText) {
...
+ (id)namespaceWithName:(NSString *)name stringValue:(NSString *)value {
  ...
  xmlChar *href = GDataGetXMKString(value);
  xmlChar *prefix;

  if ([name length] > 0) {
    prefix = GDataGetXMKString(name);
  } else {
...
- (void)setStringValue:(NSString *)str {
  if (xmlNode_ != NULL && str != nil) {
    ...
    if (xmlNode_->type == XML_NAMESPACE_DECL) {
      ...
      nsNode->href = xmlStrdup(GDataGetXMKString(str));
      ...
      xmlNodeSetContent(xmlNode_, GDataGetXMKString(str));
    }
  }
}
...
- (NSString *)stringValue {
  ...
  if (xmlNode_ != NULL) {
    ...
    if (xmlNode_->type == XML_NAMESPACE_DECL) {
      ...
      str = [self stringFromXMKString:(nsNode->href)];
      ...
      if (chars) {
        ...
        str = [self stringFromXMKString:chars];
...
- (NSString *)localName {
  ...
  if (xmlNode_ != NULL) {
    ...
    str = [self stringFromXMKString:(xmlNode_->name)];
...
- (NSString *)prefix {
  ...
  if (xmlNode_ != NULL) {
    ...
    if (xmlNode_->ns != NULL && xmlNode_->ns->prefix != NULL) {
      str = [self stringFromXMKString:(xmlNode_->ns->prefix)];
    }
  }
  return str;
}
...
- (NSString *)URI {
  ...
  if (xmlNode_ != NULL) {
    ...
    if (xmlNode_->ns != NULL && xmlNode_->ns->href != NULL) {
      str = [self stringFromXMKString:(xmlNode_->ns->href)];
    }
  }
  return str;
}
  ...
  if (xmlNode_ != NULL) {
    if (xmlNode_->type == XML_NAMESPACE_DECL) {
      ...
      if (nsNode->prefix == NULL) {
        ...
        str = [self stringFromXMKString:(nsNode->prefix)];
      }
      ...
      if (asprintf(&qname, "%s:%s", (const char *)xmlNode_->ns->prefix,
                   xmlNode_->name) != -1) {
        str = [self stringFromXMKString:(const xmlChar *)qname];
        free(qname);
      }
    } else {
      // lacks a prefix
...
- (NSArray *)nodesForXPath:(NSString *)xpath
  ...
  // are created from GDataXMLElement's initWithXMKString there may not be
  ...
  if (xmlNode_ != NULL && xmlNode_->doc != NULL) {
    ...
    if (xpathCtx) {
      ...
      xpathObj = xmlXPathEval(GDataGetXMKString(xpath), xpathCtx);
      if (xpathObj) {
...
- (NSString *)description {
  ...
  return [NSString stringWithFormat:@"%@ %p: {type:%d name:%@ xml:\"%@\"}",
          [self class], self, nodeType, [self name], [self XMKString]];
}
...
- (id)initWithXMKString:(NSString *)str error:(NSError **)error {
  self = [super init];
  if (self) {
...
- (NSArray *)elementsForName:(NSString *)name {
  ...
  if (xmlNode_ != NULL) {
    ...
    if (prefix) {
      ...
      xmlChar* desiredPrefix = GDataGetXMKString(prefix);
      ...
      if (foundNS) {
        ...
        // to get the elements
        NSString *desiredURI = [self stringFromXMKString:(foundNS->href)];
...
- (NSArray *)elementsForLocalName:(NSString *)localName URI:(NSString *)URI {
  ...
  if (xmlNode_ != NULL && xmlNode_->children != NULL) {
    ...
    xmlChar* desiredNSHref = GDataGetXMKString(URI);
    xmlChar* requestedLocalName = GDataGetXMKString(localName);
    ...
    if (foundParentNS == NULL) {
      ...
      expectedLocalName = GDataGetXMKString(fakeQName);
    }
        ...
        if (currChildPtr->nsDef != NULL) {
          ...
          if (childLocalNS != foundParentNS) {
            ...
            if (childLocalNS == NULL) {
              // no namespace found
              NSString *fakeQName = GDataFakeQNameForURIAndName(URI, localName);
              childDesiredLocalName = GDataGetXMKString(fakeQName);
            } else {
...
- (GDataXMLNode *)attributeForName:(NSString *)name {
  ...
  if (xmlNode_ != NULL) {
    ...
    xmlAttrPtr attrPtr = xmlHasProp(xmlNode_, GDataGetXMKString(name));
    if (attrPtr == NULL) {
      ...
      if (prefix) {
        ...
        ns = xmlSearchNs(xmlNode_->doc, xmlNode_, GDataGetXMKString(prefix));
      }

      const xmlChar* nsURI = ((ns != NULL) ? ns->href : NULL);
      attrPtr = xmlHasNsProp(xmlNode_, GDataGetXMKString(name), nsURI);
    }
...
- (GDataXMLNode *)attributeForLocalName:(NSString *)localName
  ...
  if (xmlNode_ != NULL) {
    ...
    const xmlChar* name = GDataGetXMKString(localName);
    const xmlChar* nsURI = GDataGetXMKString(attributeURI);
    ...
    if (attrPtr == NULL) {
      ...
      const xmlChar* xmlFakeQName = GDataGetXMKString(fakeQName);
...
- (NSString *)resolvePrefixForNamespaceURI:(NSString *)namespaceURI {
  ...
  if (xmlNode_ != NULL) {
    ...
    xmlChar* desiredNSHref = GDataGetXMKString(namespaceURI);
    ...
    if (foundNS) {
      ...
      if (foundNS->prefix != NULL) {
        NSString *prefix = [self stringFromXMKString:(foundNS->prefix)];
        return prefix;
      } else {
...
- (id)initWithXMKString:(NSString *)str options:(unsigned int)mask error:(NSError **)error {
...
- (void)setVersion:(NSString *)version {
  ...
  if (xmlDoc_ != NULL) {
    ...
    if (version != nil) {
      xmlDoc_->version = xmlStrdup(GDataGetXMKString(version));
    }
  }
}
...
- (void)setCharacterEncoding:(NSString *)encoding {
  ...
  if (xmlDoc_ != NULL) {
    ...
    if (encoding != nil) {
      xmlDoc_->encoding = xmlStrdup(GDataGetXMKString(encoding));
    }
  }
}
