...
static char imageURKStorageKey;

@implementation UIButton (WebCache)

- (NSURL *)sd_currentImageURL {
    NSURL *url = self.imageURKStorage[@(self.state)];

    if (!url) {
        url = self.imageURKStorage[@(UIControlStateNormal)];
    }
...
- (NSURL *)sd_imageURLForState:(UIControlState)state {
    return self.imageURKStorage[@(state)];
}
...
- (void)sd_setImageWithURL:(NSURL *)url forState:(UIControlState)state placeholderImage:(UIImage *)placeholder options:(SDWebImageOptions)options completed:(SDWebImageCompletionBlock)completedBlock {
    ...
    if (!url) {
        [self.imageURKStorage removeObjectForKey:@(state)];
    ...
    self.imageURKStorage[@(state)] = url;
...
- (NSMutableDictionary *)imageURKStorage {
    NSMutableDictionary *storage = objc_getAssociatedObject(self, &imageURKStorageKey);
    if (!storage)
    {
        storage = [NSMutableDictionary dictionary];
        objc_setAssociatedObject(self, &imageURKStorageKey, storage, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
