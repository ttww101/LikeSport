...
@interface SMPagerTabView()<UITextFieldDelegate>
...
//        [self setNeedsDisplay];
    [self setNeedsLayout]; //  标记为需要重新布局，异步调用layoutIfNeeded刷新布局，不立即刷新
    [self layoutSubviews];
}
...
- (void)selectTabWithIndex:(NSUInteger)index animate:(BOOL)isAnimate {
    ...
    if ([self.delegate respondsToSelector:@selector(whenSelectOnPager:)]) {
        ...
        // 切换选项卡调用相应控制器的视图出现方法
//        UIViewController* vc = [self.delegate pagerViewOfPagers:self indexOfPagers:index];
...
- (UIView *)tabView {
    if (!_tabView) {
        self.tabView = [[UIView alloc]initWithFrame:CGRectMake(0, 40, self.width, _tabFrameHeight)];
...
- (UIColor *)tabButtonTitleColorForNormal {
    if (!_tabButtonTitleColorForNormal) {
        self.tabButtonTitleColorForNormal = KSBlue;
    }
    return _tabButtonTitleColorForNormal;
}
- (UIColor *)tabButtonTitleColorForSelected {
    if (!_tabButtonTitleColorForSelected) {
        self.tabButtonTitleColorForSelected = KSBlue;
    }
    return _tabButtonTitleColorForSelected;
}
- (UIView *)selectedLine {
    if (!_selectedLine) {
        self.selectedLine = [[UIView alloc] initWithFrame:CGRectMake(0, self.tabView.height + 38, self.selectedLineWidth, 2)];
        _selectedLine.backgroundColor = [UIColor colorWithRed:254/255.0 green:198/255.0 blue:22/255.0 alpha:1];
...
- (UIScrollView*)bodyScrollView {
    if (!_bodyScrollView) {
        self.bodyScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, _tabFrameHeight+10, self.width, self.height - _tabFrameHeight)];
