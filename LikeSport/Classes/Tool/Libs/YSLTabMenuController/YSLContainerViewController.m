...
static const CGFloat kYSKScrollMenuViewHeight = 30;

@interface YSLContainerViewController () <UIScrollViewDelegate, YSKScrollMenuViewDelegate>

@property (nonatomic, assign) CGFloat topBarHeight;
@property (nonatomic, assign) NSInteger currentIndex;
@property (nonatomic, strong) YSKScrollMenuView *menuView;
...
- (void)viewDidLoad {
    ...
    _contentScrollView.frame = CGRectMake(0,_topBarHeight + kYSKScrollMenuViewHeight, self.view.frame.size.width, self.view.frame.size.height - (_topBarHeight + kYSKScrollMenuViewHeight));
    ...
    for (int i = 0; i < self.childControllers.count; i++) {
        ...
        if ([obj isKindOfClass:[UIViewController class]]) {
            ...
            controller.view.frame = CGRectMake(i * scrollWidth, -30, scrollWidth, scrollHeght);
            [_contentScrollView addSubview:controller.view];
        }
    }
    // meunView
    _menuView = [[YSKScrollMenuView alloc]initWithFrame:CGRectMake(0, _topBarHeight, self.view.frame.size.width, kYSKScrollMenuViewHeight)];
...
#pragma mark -- YSKScrollMenuView Delegate
