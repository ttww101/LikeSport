...
static const CGFloat kYSKScrollMenuViewWidth  = 90;
static const CGFloat kYSKScrollMenuViewMargin = 10;
static const CGFloat kYSLIndicatorHeight = 3;

@interface YSKScrollMenuView ()
...
@implementation YSKScrollMenuView
...
- (void)setItemTitleArray:(NSArray *)itemTitleArray
{
    if (_itemTitleArray != itemTitleArray) {
        ...
        for (int i = 0; i < itemTitleArray.count; i++) {
            CGRect frame = CGRectMake(0, 0, kYSKScrollMenuViewWidth, CGRectGetHeight(self.frame));
        ...
        _indicatorView.frame = CGRectMake(10, _scrollView.frame.size.height - kYSLIndicatorHeight, kYSKScrollMenuViewWidth, kYSLIndicatorHeight);
...
- (void)setIndicatorViewFrameWithRatio:(CGFloat)ratio isNextItem:(BOOL)isNextItem toIndex:(NSInteger)toIndex
{
    ...
    if (isNextItem) {
        indicatorX = ((kYSKScrollMenuViewMargin + kYSKScrollMenuViewWidth) * ratio ) + (toIndex * kYSKScrollMenuViewWidth) + ((toIndex + 1) * kYSKScrollMenuViewMargin);
    } else {
        indicatorX =  ((kYSKScrollMenuViewMargin + kYSKScrollMenuViewWidth) * (1 - ratio) ) + (toIndex * kYSKScrollMenuViewWidth) + ((toIndex + 1) * kYSKScrollMenuViewMargin);
    }
    
    if (indicatorX < kYSKScrollMenuViewMargin || indicatorX > self.scrollView.contentSize.width - (kYSKScrollMenuViewMargin + kYSKScrollMenuViewWidth)) {
        return;
    }
    _indicatorView.frame = CGRectMake(indicatorX, _scrollView.frame.size.height - kYSLIndicatorHeight, kYSKScrollMenuViewWidth, kYSLIndicatorHeight);
