...
- (void)setupTableView {
    [self.tableView registerNib:[UINib nibWithNibName:@"KSWikiTableViewCell" bundle:nil] forCellReuseIdentifier:@"useid"];
...
- (void)setData {
    ...
    [KSKuaiShouTool getWikiWithType:-1 withCompleted:^(id result) {
...
//        }
        last.result = [[WikiResult alloc] init];
        
        
        NSMutableArray *mutaArr = [[NSMutableArray alloc] init];
        for (NSDictionary *news in result[@"data"]) {
            WikiResult *w1 = [[WikiResult alloc] init];
            w1.matchtypefullname = news[@"title"];
            w1.cteamname = news[@"guestteam"];
            w1.hteamname = news[@"hometeam"];
            w1.content = news[@"content"];
//            w1.starttime = [news[@"mtime"] longLongValue] / 1000;
            NSDate *date = [NSDate date];
            w1.starttime = [date timeIntervalSince1970];;
            
            [mutaArr addObject:w1];
        }
        
        
        
        last.result.t0 = [mutaArr copy];
...
//    [KSLikeSportTool getLiveWithCompleted:^(id obj) {
//        KSLastestParamResult *result = obj;
//        
//        
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    //    KSLastestParamResult *result = self.liveGroup[section];
    //    return self.liveGroup.count;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    KSWikiTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:reuseid forIndexPath:indexPath];
...
- (UITableView *)tableView {
    if (!_tableView) {
        UITableView *tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, kSceenWidth, kSceenHeight-70-42) style:UITableViewStylePlain];
        if ([self isNotchScreen]) tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, kSceenWidth, kSceenHeight-87-83) style:UITableViewStylePlain];
