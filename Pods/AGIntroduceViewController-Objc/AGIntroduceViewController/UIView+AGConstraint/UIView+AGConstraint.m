...
- (UIView *)constraintsTopLayoutGuide:(UIViewController *)vc toLayoutAttribute:(NSLayoutAttribute)topLayoutAttribute constant:(CGFloat)constant __attribute__((warn_unused_result)) {
    if (!self.superview) { return self; }
    if (self.translatesAutoresizingMaskIntoConstraints) self.translatesAutoresizingMaskIntoConstraints = NO;
    if (vc) [self.superview addConstraint:[NSLayoutConstraint constraintWithItem:self attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:vc.view.safeAreaLayoutGuide.topAnchor attribute:topLayoutAttribute multiplier:1.0 constant:constant]];
    return self;
}
...
- (UIView *)constraintsBottomLayoutGuide:(UIViewController *)vc toLayoutAttribute:(NSLayoutAttribute)bottomLayoutAttribute constant:(CGFloat)constant __attribute__((warn_unused_result)) {
    if (!self.superview) { return self; }
    if (self.translatesAutoresizingMaskIntoConstraints) self.translatesAutoresizingMaskIntoConstraints = NO;
    if (vc) [self.superview addConstraint:[NSLayoutConstraint constraintWithItem:self attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:vc.view.safeAreaLayoutGuide.bottomAnchor attribute:bottomLayoutAttribute multiplier:1.0 constant:constant]];
    return self;
}
